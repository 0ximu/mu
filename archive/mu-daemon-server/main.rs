...
async fn main() -> Result<()> {
    ...
    let mubase_path = cli.mubase.unwrap_or_else(|| {
        ...
        if legacy_path.exists() {
            info!("Using legacy .mubase path. Consider running 'mu migrate' to move to .mu/mubase");
            return legacy_path;
        }
    ...
    let state = AppState {
        ...
        // Embedding model is optional - semantic search requires external configuration
        // To enable embeddings, set up the embedding model in your configuration
        embedding_model: {
            // Log that embeddings are not configured
            info!("Embedding model not configured - semantic search will be unavailable");
            info!("To enable embeddings, configure an embedding model provider");
            None
        },
    };
