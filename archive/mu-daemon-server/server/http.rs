...
async fn get_node(
    ...
    let mubase = match state.projects.get_mubase(params.cwd.as_deref()).await {
        Ok(m) => m,
        Err(e) => return ApiResponse::<NodeResponse>::err(e, start.elapsed().as_millis() as u64),
    };
...
async fn get_neighbors(
    ...
    // Query neighbors based on direction using parameterized queries to prevent SQL injection
    let query_result = match params.direction.as_str() {
        "outgoing" => {
            let sql = "SELECT n.id, n.type, n.name, n.qualified_name, n.file_path, n.line_start, n.line_end, n.complexity
             FROM edges e
             JOIN nodes n ON e.target_id = n.id
             WHERE e.source_id = ?";
            mubase.query_with_params(sql, &[&id])
        }
        "incoming" => {
            let sql = "SELECT n.id, n.type, n.name, n.qualified_name, n.file_path, n.line_start, n.line_end, n.complexity
             FROM edges e
        ...
        _ => {
            let sql = "SELECT DISTINCT n.id, n.type, n.name, n.qualified_name, n.file_path, n.line_start, n.line_end, n.complexity
             FROM edges e
             JOIN nodes n ON (e.target_id = n.id OR e.source_id = n.id)
             WHERE (e.source_id = ? OR e.target_id = ?) AND n.id != ?";
            mubase.query_with_params(sql, &[&id, &id, &id])
        }
    };
...
async fn scan(
    ...
    match mu_core::scanner::scan_directory_sync(
...
async fn export_graph(
    ...
    let node_filter: Option<Vec<String>> = params
        .nodes
    ...
    let param_refs: Vec<&dyn duckdb::ToSql> =
        all_params.iter().map(|s| s as &dyn duckdb::ToSql).collect();

    let nodes_result = match mubase.query_with_params(&nodes_sql, &param_refs) {
        Ok(r) => r,
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    // Query edges - for filtered exports, we need to duplicate parameters for subqueries
    ...
    // For edges query, we need to duplicate params for both subqueries
    let edge_params: Vec<String> = if node_filter.is_some() || type_filter.is_some() {
        let mut p = all_params.clone();
        p.extend(all_params.clone());
        p
    } else {
        Vec::new()
    };
    let edge_param_refs: Vec<&dyn duckdb::ToSql> = edge_params
        .iter()
        .map(|s| s as &dyn duckdb::ToSql)
        .collect();

    let edges_result = match mubase.query_with_params(&edges_sql, &edge_param_refs) {
        Ok(r) => r,
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let (content, content_type) = match params.format.to_lowercase().as_str() {
        ...
        _ => {
            return (
                ...
                [(axum::http::header::CONTENT_TYPE, "text/plain")],
                format!(
                    "Unknown format: {}. Supported: json, mu, mermaid, d2, cytoscape",
                    params.format
                ),
            )
                .into_response();
    ...
    match result {
        Ok(ctx) => {
            ...
            match omega_result {
                ...
                Err(e) => {
                    ApiResponse::<OmegaContextResponse>::err(e, start.elapsed().as_millis() as u64)
                }
            }
        }
...
"#
    ...
    Ok(OmegaContextResponse {
        ...
        body_tokens,
        total_tokens: if req.include_seed {
            total_tokens
        } else {
            body_tokens
        },
        ...
        if count >= 3 {
            patterns.push(PatternInfo {
                ...
                anti_patterns: vec![format!(
                    "Using generic names without '{}' suffix for this type",
                    suffix
                )],
            });
        }
...
fn detect_architecture_patterns(
    ...
    let service_count = service_result
        .rows
        .first()
        .and_then(|r| r.first())
        .and_then(|v| v.as_i64())
    ...
    let controller_count = controller_result
        .rows
        .first()
        .and_then(|r| r.first())
        .and_then(|v| v.as_i64())
...
fn detect_testing_patterns(mubase: &crate::storage::MUbase) -> Result<Vec<PatternInfo>, String> {
    ...
    let test_func_count = test_func_result
        .rows
        .first()
        .and_then(|r| r.first())
        .and_then(|v| v.as_i64())
    ...
    if test_func_count >= 3 {
        patterns.push(PatternInfo {
            ...
            category: "testing".to_string(),
            description: format!(
                "Test functions with 'test' prefix ({} found)",
                test_func_count
            ),
...
async fn analyze_warnings(state: &AppState, target: &str) -> Result<WarnResponse, String> {
    ...
    if target.starts_with("mod:") || target.starts_with("cls:") || target.starts_with("fn:") {
        ...
        // Try to find by file path or name using parameterized query
        let path_pattern = format!("%{}", target);
        let search_sql = "SELECT id, type FROM nodes WHERE file_path LIKE ? OR name = ? LIMIT 1";
        if let Ok(result) = mubase.query_with_params(search_sql, &[&path_pattern, &target]) {
    ...
    // Check impact (dependents count) using parameterized query
    if let Some(ref id) = node_id {
        let impact_sql = "SELECT COUNT(*) FROM edges WHERE target_id = ?";
        if let Ok(result) = mubase.query_with_params(impact_sql, &[id]) {
            let dependent_count = result
                .rows
                .first()
                .and_then(|r| r.first())
                .and_then(|v| v.as_i64())
            ...
            if dependent_count > 10 {
                let level = if dependent_count > 30 {
                    "error"
                } else {
                    "warn"
                };
                warnings.push(WarningInfo {
                    ...
                    message: format!(
                        "{} nodes depend on this - changes may have wide impact",
                        dependent_count
                    ),
    ...
    // Check complexity using parameterized query
    if let Some(ref id) = node_id {
        let complexity_sql = "SELECT complexity FROM nodes WHERE id = ?";
        if let Ok(result) = mubase.query_with_params(complexity_sql, &[id]) {
            ...
            if complexity > 50 {
                warnings.push(WarningInfo {
                    ...
                    message: format!(
                        "Very high complexity ({}) - consider refactoring before changes",
                        complexity
                    ),
                    details: Some(serde_json::json!({
    ...
    let security_keywords = [
        "auth", "password", "token", "secret", "crypto", "encrypt", "session", "login",
    ];
    ...
    if let Some(ref id) = node_id {
        if id.starts_with("mod:") && !id.contains("test") {
            ...
            let test_patterns = vec![format!("test_{}", stem), format!("{}_test", stem)];
            ...
            for pattern in test_patterns {
                ...
                if let Ok(result) = mubase.query_with_params(test_sql, &[&like_pattern]) {
                    let count = result
                        .rows
                        .first()
                        .and_then(|r| r.first())
...
fn generate_warning_summary(warnings: &[WarningInfo], target: &str) -> String {
    ...
    let categories: std::collections::HashSet<&str> =
        warnings.iter().map(|w| w.category.as_str()).collect();
