...
use super::parser;
use super::planner::{
    self, AnalysisOperation, ExecutionPlan, GraphOpType, GraphOperation, SchemaOperation,
};
...
async fn resolve_node_id(input: &str, state: &AppState) -> String {
    ...
    if input.starts_with("mod:")
        || input.starts_with("cls:")
        || input.starts_with("fn:")
        || input.starts_with("ext:")
    {
        let mubase = state.mubase.read().await;
    ...
    // Try to find by name using parameterized query to prevent SQL injection
    let mubase = state.mubase.read().await;
    let name_pattern = format!("%{}", input);
    let sql = "SELECT id FROM nodes WHERE name = ? OR qualified_name LIKE ? LIMIT 1";

    if let Ok(result) = mubase.query_with_params(sql, &[&input, &name_pattern]) {
        ...
        || input.ends_with(".rs")
    {
        ...
        let path_pattern = format!("%{}", normalized);
        let sql = "SELECT id FROM nodes WHERE file_path = ? OR file_path LIKE ? LIMIT 1";

        if let Ok(result) = mubase.query_with_params(sql, &[&normalized, &path_pattern]) {
            if let Some(row) = result.rows.first() {
                if let Some(id) = row.first().and_then(|v| v.as_str()) {
...
async fn execute_graph(op: GraphOperation, state: &AppState) -> Result<QueryResult> {
    ...
    match op.op_type {
        GraphOpType::Cycles => {
            ...
            // Format as result - each cycle is a row with node IDs as columns
            Ok(QueryResult {
                columns: vec!["cycle".to_string()],
                rows: cycles
                    .into_iter()
                    .map(|c| {
                        // Return the cycle as a comma-separated string for easy display
                        vec![serde_json::Value::String(c.join(" → "))]
                    })
                    .collect(),
            })
        }
...
async fn execute_analysis(op: AnalysisOperation, state: &AppState) -> Result<QueryResult> {
    ...
    match op.analysis_type {
        parser::AnalysisType::Complexity => {
            // Get nodes ordered by complexity using parameterized query
            if let Some(target) = &op.target {
                let pattern = format!("%{}%", target);
                let sql = "SELECT id, name, complexity FROM nodes WHERE file_path LIKE ? ORDER BY complexity DESC LIMIT 50";
                mubase.query_with_params(sql, &[&pattern])
            } else {
                let sql = "SELECT id, name, complexity FROM nodes WHERE complexity > 0 ORDER BY complexity DESC LIMIT 50";
                mubase.query(sql)
            }
        }
        ...
        parser::AnalysisType::Circular => {
            ...
            Ok(QueryResult {
                ...
                rows: cycles
                    .into_iter()
                    .map(|c| {
                        // Return the cycle as a string for easy display
                        vec![serde_json::Value::String(c.join(" → "))]
                    })
                    .collect(),
            })
...
async fn execute_schema(op: SchemaOperation, state: &AppState) -> Result<QueryResult> {
    match op {
        ...
        SchemaOperation::ListColumns(node_type) => Ok(QueryResult {
            columns: vec![
