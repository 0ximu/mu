...
fn plan_select(q: SelectQuery) -> ExecutionPlan {
    ...
    if let Some(having) = &q.having_clause {
        sql.push_str(" HAVING ");
        let having_conds: Vec<String> = having.comparisons.iter().map(format_comparison).collect();
...
fn format_comparison(comp: &Comparison) -> String {
    let op_str = match comp.op {
        ...
        ComparisonOp::Gte => ">=",
        ComparisonOp::Lte => "<=",
        ComparisonOp::Like => "ILIKE", // Case-insensitive for better UX
        ComparisonOp::In => "IN",
        ComparisonOp::NotIn => "NOT IN",
    };

    // Handle 'type' field specially - normalize to lowercase for case-insensitive matching
    // Database stores types as lowercase: 'function', 'class', 'module', 'external'
    if comp.field == "type" {
        let normalized_value = match &comp.value {
            Value::String(s) => Value::String(s.to_lowercase()),
            other => other.clone(),
        };
        if matches!(comp.op, ComparisonOp::In | ComparisonOp::NotIn) {
            let value_str = format_value(&normalized_value);
            return format!("{} {} ({})", comp.field, op_str, value_str);
        } else {
            let value_str = format_value(&normalized_value);
            return format!("{} {} {}", comp.field, op_str, value_str);
        }
    ...
    } else if matches!(comp.op, ComparisonOp::Like) {
        // ILIKE needs % wildcards for substring matching
        let pattern = match &comp.value {
            Value::String(s) => format!("'%{}%'", s.replace('\'', "''")),
            _ => format_value(&comp.value),
        };
        format!("{} {} {}", comp.field, op_str, pattern)
    } else {
        let value_str = format_value(&comp.value);
        format!("{} {} {}", comp.field, op_str, value_str)
    }
...
fn plan_show(q: ShowQuery) -> ExecutionPlan {
    ...
    let (op_type, edge_types) = match q.show_type {
        ...
        ShowType::Inheritance => (
            GraphOpType::Dependencies,
            Some(vec!["inherits".to_string()]),
        ),
...
fn plan_describe(q: DescribeQuery) -> ExecutionPlan {
    match q.target {
        ...
        DescribeTarget::NodeType(nt) => {
            ExecutionPlan::Schema(SchemaOperation::DescribeNodeType(nt))
        }
    }
}
