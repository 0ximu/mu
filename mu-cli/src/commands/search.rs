...
pub async fn run(
    ...
    run_direct(query, limit, threshold, rerank, expand, format, start).await
}
...
async fn run_direct(
    ...
    let results = if has_embeddings {
        ...
        // Step 1: Get initial results (expand or plain search)
        ...
        let mut candidates = if expand {
            run_expanded_search(&mubase, query, fetch_limit, threshold)?
        } else {
            run_semantic_search(&mubase, query, fetch_limit, threshold)?
        };

        // Step 2: Rerank using graph signals if enabled
